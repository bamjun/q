...
github_repo_version=$(curl -s "https://bamjun.github.io/q/env_createTempForder.sh" | bash -s -- check_version)
...
if [ "$version_index" != "$github_repo_version" ]; then
    ...
    if [ "$choose_index" == "1" ]; then
        ...
        curl -o $script_path "https://bamjun.github.io/q/env_createTempForder.sh"
        exit 1
